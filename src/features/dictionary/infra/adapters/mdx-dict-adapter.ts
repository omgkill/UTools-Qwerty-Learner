...
import type { MdxDictLoader } from '../../application/ports'
import type { DictMeta, WordInfo } from '../../domain'
...
export class MdxDictAdapter extends BaseDictAdapter {
  ...
  constructor(id: string, name: string, private _path: string, private dictLoader?: MdxDictLoader) {
    super()
    this._id = id
    this._name = name
  }
  ...
  async load(): Promise<void> {
    ...
    if (this.dictLoader) {
      const result = await this.dictLoader.load(this._path)
      this._mdxLookup = result.mdxLookup
      this._mddLookup = result.mddLookup ?? null
    }
  ...
  private _extractTranslations(html: string): string[] {
    ...
    for (const line of lines) {
      ...
      if (trimmed && trimmed.length > 1 && trimmed.length < 200 && /[\u4e00-\u9fa5]/.test(trimmed)) {
  ...
  private async _replaceResources(html: string, mddLookup: (resource: string) => Promise<Buffer>): Promise<string> {
    if (!html) return html
