...
import { currentChapterAtom, currentWordBankIdAtom } from '@/store'
...
export default function ChapterList() {
  ...
  const [checkedChapter, setCheckedChapter] = useState(wordBank?.id === currentWordBankId ? currentChapter : 0)
  const navigate = useNavigate()

  const chapterCount = calcChapterCount(wordBank?.length ?? 0)
  const showChapterList = wordBank !== null

  useEffect(() => {
    if (wordBank) {
      setCheckedChapter(wordBank.id === currentWordBankId ? currentChapter : 0)
    }
  }, [currentChapter, currentWordBankId, wordBank])
  ...
  const onConfirm = useCallback(() => {
    if (wordBank) {
      ...
      setState((state) => {
        state.chapterListWordBank = null
      })
      navigate('/')
    }
  }, [checkedChapter, wordBank, navigate, setCurrentChapter, setCurrentWordBankId, setState])
                ...
                {wordBank && (
                  <>
                    <div className="flex w-full items-end justify-between py-4 pl-5">
                      <span className="text-lg text-gray-700 dark:text-gray-200">{wordBank.name}</span>
